...
export default function MusicTab({
  ...
  saved?: boolean;
  error?: string;
  /** Products this account has put through the shop's review pipeline. */
  ...
  if (!hasMusicAccess) {
    if (profile) {
      return (
        <div className="border-flame-2/40 bg-flame-2/10 rounded-xl border px-5 py-4 text-sm text-muted">
          Your Music Collective application for{" "}
    ...
    return (
      <div className="border-flame-2/40 bg-flame-2/10 rounded-xl border px-5 py-4 text-sm text-muted">
        Join the WHOA Music Collective to get a Music tab here — bio, links, and
        everything else, editable any time.{" "}
        <Link
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="card-surface rounded-xl p-6">
        ...
        <form
          action={saveMusicianProfileAction}
      ...
      {/* Vinyl, tapes, merch — a musician's product goes through the same
          review queue and reaches the same shop as an artist's. */}
      <ProductPipeline
        code={code}
        products={submissions}
        blurb="Vinyl, tapes, merch — submit up to 5 at once. Each one goes to WHOA for review before it shows in the shop."
        submitted={submitted}
        error={submitError}
